...
如果我们在函数的形参列表里的最后一个变量前，插入 ``&rest`` 符号，那麽当这个函数被调用时，这个变量会被设成一个带有剩馀参数的列表。现在我们可以明白 ``funcall`` 是如何根据 ``apply`` 写成的。它或许可以定义成：
...
那麽在 ``&optional`` 之后的参数都是选择性的，缺省为 ``nil`` :
...
有一种函数你不能作为参数传给 ``compile`` ：一个像是 ``stamp`` 或是 ``reset`` 这种，在顶层明确使用词法上下文输入的函数 (即 ``let`` ) [3]_ 在一个文件里面定义这些函数，接着编译然后载入文件是可以的。这么限制直译的代码的是实作的原因，而不是因为在词法上下文里明确定义函数有什麽问题。
